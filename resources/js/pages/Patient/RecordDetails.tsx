...
interface MedicalRecordDetails {
    ...
    [key: string]:
        ...
        | number
        | string[]
        | Record<string, string | number | { value: string | number; status?: string; range?: string }>
...
export default function RecordDetails({ user, record }: RecordDetailsProps) {
    ...
    useEffect(() => {
        ...
        if (typeof record.details === 'string') {
            try {
                const parsedDetails = JSON.parse(record.details);
                if (parsedDetails.address || (parsedDetails.patient_info && parsedDetails.patient_info.address)) {
            ...
            if (record.details.address || (record.details.patient_info && record.details.patient_info.address)) {
                // If details already has address info, don't make the API call
    ...
    const patientAddress = (): string => {
        ...
        if (typeof record.details === 'string') {
            try {
                ...
                if (parsedDetails.address && typeof parsedDetails.address === 'string' && parsedDetails.address.trim() !== '') {
                ...
                if (
                    ...
                    parsedDetails.patient_info.address &&
        ...
        if (patientDetails && patientDetails.address && typeof patientDetails.address === 'string' && patientDetails.address.trim() !== '') {
        ...
        if (typeof record.details === 'object') {
            ...
            if (details.doctor_name || details.attending_physician) {
                setDoctorInfo({
                    ...
                    specialization:
                        details.doctor_specialization || details.specialty ? String(details.doctor_specialization || details.specialty) : null,
                });
    ...
    const getDoctorDisplay = () => {
        ...
        if (record.assignedDoctor && record.assignedDoctor.name) {
            return `Dr. ${record.assignedDoctor.name}${
                record.assignedDoctor.doctorProfile?.specialization ? `, ${record.assignedDoctor.doctorProfile.specialization}` : ''
            }`;
        }
        ...
        if (typeof record.details === 'object') {
            ...
            if (details.doctor_name || details.attending_physician) {
                return `Dr. ${details.doctor_name || details.attending_physician}${
                    details.doctor_specialization || details.specialty ? `, ${details.doctor_specialization || details.specialty}` : ''
                }`;
            }
        }
    ...
    const renderPrescriptionItem = (item: string | PrescriptionItemInterface | Record<string, unknown>, index: number) => {
        if (typeof item === 'string') {
            return (
                <tr key={index}>
                    <td className="border border-gray-200 p-3">{item}</td>
            ...
            return (
                <tr key={index}>
                    <td className="border border-gray-200 p-3">{itemObj.medication || '-'}</td>
                    <td className="border border-gray-200 p-3">{itemObj.dosage || '-'}</td>
                    <td className="border border-gray-200 p-3">{itemObj.frequency || '-'}</td>
                    <td className="border border-gray-200 p-3">{itemObj.duration || '-'}</td>
                    <td className="border border-gray-200 p-3">{itemObj.instructions || '-'}</td>
                </tr>
            );
        }
    };
    ...
    return (
        ...
        `}</style>
                ...
                <main className="container mx-auto px-4 py-6 md:px-6 print:m-0 print:overflow-visible print:bg-white print:p-0 print:dark:bg-white">
                    ...
                    <div className="mb-6 flex items-center print:hidden">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">{isLabRecord ? 'Laboratory Results' : 'Medical Record'}</h1>
                            <p className="text-gray-500">
                                Viewing your {isLabRecord ? 'lab results' : 'medical record'} from {formatDate(record.appointment_date)}
                            </p>
                        </div>
                        <div className="ml-auto">
                            <Button variant="outline" onClick={handlePrint} className="flex items-center gap-1" disabled={isLabRecord}>
                    ...
                    <Card className="border-t-4 border-t-blue-500 print:hidden print:border-none print:shadow-none">
                        <CardContent className="p-0">
                            ...
                            <div className="border-b border-gray-200 p-6">
                                <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
                                    ...
                                    <div className="space-y-6">
                                        <div>
                                            <h3 className="mb-1 text-sm text-gray-500">Patient Name</h3>
                                            <p className="font-medium">{user?.name || 'Unknown Patient'}</p>
                                        </div>
                                        <div>
                                            <h3 className="mb-1 text-sm text-gray-500">Address</h3>
                                            <p className="inline-block rounded bg-blue-50 px-2 py-0.5 font-medium text-blue-700">
                                                {patientAddress()}
                                            </p>
                                        </div>
                                        <div>
                                            <h3 className="mb-1 text-sm text-gray-500">Record Type</h3>
                                            <p className="font-medium text-blue-600">{getRecordTypeDisplay(record.record_type)}</p>
                                        </div>
                                        <div>
                                            <h3 className="mb-1 text-sm text-gray-500">Status</h3>
                                            <div className="inline-block rounded-full bg-gray-100 px-2 py-1 text-xs font-medium text-gray-800 lowercase">
                                    ...
                                    <div className="space-y-6">
                                        <div>
                                            <h3 className="mb-1 text-sm text-gray-500">Appointment Date</h3>
                                            <p className="font-medium">{formatDate(record.appointment_date)}</p>
                                        </div>
                                        <div>
                                            <h3 className="mb-1 text-sm text-gray-500">Appointment Time</h3>
                                            <p className="font-medium">{formatTime(details.appointment_time) || 'Not specified'}</p>
                                        </div>
                                        <div>
                                            <h3 className="mb-1 text-sm text-gray-500">Doctor</h3>
                                            <p className="font-medium">{getDoctorDisplay()}</p>
                                        </div>
                                        <div>
                                            <h3 className="mb-1 text-sm text-gray-500">Created On</h3>
                                    ...
                                    <div className="space-y-6">
                                        <div>
                                            <h3 className="mb-1 text-sm text-gray-500">Follow-up Date</h3>
                                            <p className="font-medium">{formatDate(details.followup_date) || 'N/A'}</p>
                                        </div>
                                        {record.reason && (
                                            <div>
                                                <h3 className="mb-1 text-sm text-gray-500">Reason for Visit</h3>
                            ...
                            <div className="p-6">
                                ...
                                {details.diagnosis && (
                                    <div className="mb-6">
                                        <h3 className="mb-2 text-lg font-bold">Diagnosis</h3>
                                ...
                                {/* Vital Signs */}
                                {details.vital_signs &&
                                    Object.keys(details.vital_signs).some(
                                        (key) =>
                                            details.vital_signs![key] !== null &&
                                            details.vital_signs![key] !== undefined &&
                                            details.vital_signs![key] !== '',
                                    ) && (
                                        <div className="mb-6">
                                            <h3 className="mb-2 text-lg font-bold">Vital Signs</h3>
                                            <div className="grid grid-cols-2 gap-4 rounded-md bg-gray-50 p-4 md:grid-cols-3">
                                ...
                                {details.prescriptions && (
                                    <div className="mb-6">
                                        <h3 className="mb-2 text-lg font-bold">Prescriptions</h3>
                                        <div className="overflow-x-auto">
                                            <table className="w-full border-collapse">
                                                <thead className="bg-gray-50">
                                                    <tr>
                                                        <th className="border border-gray-200 p-3 text-left">Medication</th>
                                                        <th className="border border-gray-200 p-3 text-left">Dosage</th>
                                                        <th className="border border-gray-200 p-3 text-left">Frequency</th>
                                                        <th className="border border-gray-200 p-3 text-left">Duration</th>
                                                        <th className="border border-gray-200 p-3 text-left">Instructions</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {(() => {
                                                        if (Array.isArray(details.prescriptions)) {
                                                            return details.prescriptions.map((item, index) => renderPrescriptionItem(item, index));
                                                            ...
                                                            return Object.entries(details.prescriptions).map(([key, value], index) => {
                                                                if (typeof value === 'object' && value !== null) {
                                                                    ...
                                                                    return (
                                                                        <tr key={index}>
                                                                            <td className="border border-gray-200 p-3">{key}</td>
                                                                            <td className="border border-gray-200 p-3">{value?.toString() || '-'}</td>
                                                                            <td className="border border-gray-200 p-3">-</td>
                                                                            <td className="border border-gray-200 p-3">-</td>
                                                                            <td className="border border-gray-200 p-3">-</td>
                                                                        </tr>
                                ...
                                {details.notes && (
                                    <div className="mb-6">
                                        <h3 className="mb-2 text-lg font-bold">Notes</h3>
                                        <div className="rounded-md bg-gray-50 p-4">
                                ...
                                {details.medical_history && (
                                    <div className="mb-6">
                                        <h3 className="mb-2 text-lg font-bold">Medical History</h3>
                                        <div className="rounded-md bg-gray-50 p-4">
                                    ...
                                    <div className="mb-6">
                                        ...
                                        <div className="overflow-x-auto">
                                            <table className="w-full border-collapse">
                                                <thead className="bg-gray-50">
                                                    <tr>
                                                        <th className="border border-gray-200 p-3 text-left">Test</th>
                                                        <th className="border border-gray-200 p-3 text-left">Result</th>
                                                        <th className="border border-gray-200 p-3 text-left">Reference Range</th>
                                                ...
                                                <tbody>
                                                    {Object.entries(details.results || {}).map(([test, result], index) => (
                                                        <tr key={index}>
                                                            <td className="border border-gray-200 p-3">{test}</td>
                                                            ...
                                                            <td className="border border-gray-200 p-3">
                                                                {typeof result === 'object' && result !== null && result.range ? result.range : '-'}
                                                            </td>
                                                            <td className="border border-gray-200 p-3">
                    ...
                    <div className="mx-auto hidden print:block" style={{ maxWidth: '800px' }}>
                        {/* Title */}
                        <div className="text-center">
                            <h1 className="text-2xl font-bold">Medical Record</h1>
                            <p className="mt-1 text-sm">Physician: {getDoctorDisplay()}</p>
                            <p className="mt-1 text-sm">Viewing medical record from {formatDate(record.appointment_date)}</p>
                        </div>
                        ...
                        <div className="mb-4 text-sm">
                            <p>
                                The following information is a comprehensive medical record of the patient, intended for professional use only. This
                                document ensures a detailed overview of the patient's medical history and current health status.
                            </p>
                        </div>
                        ...
                        <div className="mb-8">
                            <table className="mb-0 w-full border-collapse">
                                <thead>
                                    <tr>
                                        <th className="w-1/3 border border-gray-300 bg-gray-50 p-2 text-left font-medium">Patient Information</th>
                                        <th className="w-2/3 border border-gray-300 bg-gray-50 p-2 text-left font-medium">Details</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td className="border border-gray-300 p-2">Name:</td>
                                        <td className="border border-gray-300 p-2">{user?.name || 'Unknown Patient'}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td className="border border-gray-300 p-2">{user?.email || details.patient_info?.email || 'Not provided'}</td>
                                    </tr>
                                    <tr>
                                        <td className="border border-gray-300 p-2">Address:</td>
                                        <td className="border border-gray-300 p-2">{patientAddress()}</td>
                                    </tr>
                                    <tr>
                                        <td className="border border-gray-300 p-2">Doctor:</td>
                                        <td className="border border-gray-300 p-2">{getDoctorDisplay()}</td>
                                    </tr>
                                    <tr>
                                        <td className="border border-gray-300 p-2">Record Type:</td>
                                        <td className="border border-gray-300 p-2">{getRecordTypeDisplay(record.record_type)}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td className="border border-gray-300 p-2">{formatDate(record.appointment_date)}</td>
                                    </tr>
                                    <tr>
                                        <td className="border border-gray-300 p-2">Appointment Time:</td>
                                        <td className="border border-gray-300 p-2">{formatTime(details.appointment_time) || 'Not specified'}</td>
                                    </tr>
                                    <tr>
                                        <td className="border border-gray-300 p-2">Created Date:</td>
                                        <td className="border border-gray-300 p-2">{formatDate(record.created_at) || 'Not available'}</td>
                                    </tr>
                                    <tr>
                                        <td className="border border-gray-300 p-2">Follow-up Date:</td>
                                        <td className="border border-gray-300 p-2">{formatDate(details.followup_date) || 'N/A'}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        {details.diagnosis && (
                            <div className="mb-4">
                                <h3 className="mb-1 text-base font-bold text-blue-800">Diagnosis</h3>
                        ...
                        {details.prescriptions && (
                            <div className="mb-4">
                                <h3 className="mb-1 text-base font-bold text-blue-800">Prescriptions</h3>
                                <table className="w-full border-collapse">
                                    <thead>
                                        <tr>
                                            <th className="border border-gray-300 bg-gray-50 p-2 text-left">Medication</th>
                                            <th className="border border-gray-300 bg-gray-50 p-2 text-left">Dosage</th>
                                            <th className="border border-gray-300 bg-gray-50 p-2 text-left">Frequency</th>
                                            <th className="border border-gray-300 bg-gray-50 p-2 text-left">Duration</th>
                                            <th className="border border-gray-300 bg-gray-50 p-2 text-left">Instructions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {(() => {
                        ...
                        {details.notes && (
                            <div className="mb-4">
                                <h3 className="mb-1 text-base font-bold text-blue-800">Notes</h3>
                                <div className="rounded border border-gray-300 p-3">
                                    <p className="whitespace-pre-line">{details.notes}</p>
                                </div>
                            </div>
                        ...
                        {details.medical_history && (
                            <div className="mb-4">
                                <h3 className="mb-1 text-base font-bold text-blue-800">Medical History</h3>
                                <div className="rounded border border-gray-300 p-3">
                                    <p className="whitespace-pre-line">{details.medical_history}</p>
                                </div>
                            </div>
